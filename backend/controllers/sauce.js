...
module.exports.likeDislike = (req, res, next) => {
    ...
    Sauce.findOne({ _id: req.params.id }) // récupération de la sauce qui nous intéresse
        .then((sauce) => {
            console.log(sauce);
            if (sauce.usersLiked.includes(userId)) { // si l'utilisateur aime déjà la sauce
                Sauce.updateOne( // modification
                    { _id: req.params.id },
                    {
                        $pull: { usersLiked: userId }, // on efface l'userId
            ...
            console.log(sauce);
            if (sauce.usersDisliked.includes(userId)) { // si l'utilisateur n'aime pas la sauce
                Sauce.updateOne( // modification
                    { _id: req.params.id },
                    {
                        $pull: { usersDisliked: userId }, // on efface l'userId
        ...
        .then(() => {
            ...
            else if (like === -1) { // si l'utilisateur veut le statut dislike
                Sauce.updateOne( // modification
                    { _id: req.params.id },
                    {
